...
7. **高级咨询风地图系统重构与交互优化** — 全面升级为低饱和度、正式、适合部长级汇报的大屏配色：
   - 地图背景采用 `#F5F2EA` 暖灰，周边遮罩采用 `#ECE8DF` 浅米灰，哈萨克斯坦主体采用 `#FBFAF5` 象牙白。
   - 国家边界采用 `#7D8C84` 灰绿极细线（`1.3px`），辅以 `#D8DDD6`（不透明度 `40%`，线宽 `3.2px`）平滑防锯齿外描边，消除硬贴纸感；国内行政边界收窄至 `0.9px` 的 `#D1D8D1` 淡灰绿。
   - 重点管网高亮为 `#6F4A28` 深色，主干管网采用 `#9C6B3C` 深棕铜色，普通线路调降至 `72%` 不透明度作为背景，外部/规划通道采用 `#9A9A9A` 灰色虚线。
   - 城市节点统一采用 `#264653` 深松石蓝；首都 Astana 采用特调 `#102A43` 深政务蓝包覆沙金色外圈。
   - 彻底移除了原先阻断性的各州名称 `<Popup>` 白气泡，还原为鼠标悬停时极其灵敏、丝滑的免弹窗无字矢量边界高亮交互。
