...
7. **高级咨询风地图系统重构与交互优化** — 全面升级为低饱和度、稳重克制、非常适合正式 PPT 与部长级汇报的配色：
   - 地图背景采用 `#E5DFD2` 稍暗灰米色，周边邻国采用 `#E5DFD2` 纯掩膜，哈萨克斯坦主体采用更亮的暖白色 `#F8F6EF`，使国家轮廓清楚浮现。
   - 国家边界采用 `#5F746D` 低饱和深灰绿色线（`1.7px`，透明度 `90%`），下方铺设极轻的 `#FFFFFF`（透明度 `35%`，线宽 `3.0px`）纯白外描边增强立体质感防止锯齿；国内行政边界收窄至极细 `0.8px` 的 `#C8CEC6` 淡灰绿（透明度 `50%`）。
   - 重点与主管线统一采用 `#8A623B` 低饱和深棕铜色，二级普通线路采用 `#A78257` 并调降至 `75%` 透明度，外部与规划线路统一采用 `#9A9A9A` 灰色虚线。
   - 城市节点图标统一使用 `#264653` 深蓝灰（Astana 包覆 `#C5A059` 细沙金圈），文字标注设为 `#2F4858` 深灰蓝，清晰易读防过黑突兀。
   - 彻底移除了原先阻断性的各州名称 `<Popup>` 白气泡，还原为鼠标悬停时极其灵敏、丝滑的免弹窗无字区划边界高亮交互。
