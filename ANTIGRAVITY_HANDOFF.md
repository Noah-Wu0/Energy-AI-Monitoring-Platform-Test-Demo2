...
- **高级咨询风配色系统（低饱和、稳重克制、适合正式 PPT/报告）**：
  - **背景底图**：稍暗的灰米色 `#E5DFD2`，消除纯白及冷灰感，并将底图地形纹理/边界不透明度降低至极其轻微的 `0.15`，彻底避免杂音干扰。
  - **国土主体**：哈萨克斯坦主体采用更亮的暖白色 `#F8F6EF`，使国家轮廓极其清晰、立体地从背景中浮现出来。
  - **双层精细国界线**：主国界采用低饱和深灰绿色 `#5F746D`（线宽控制在极富质感的 `1.7px`，透明度 `90%`）；在其下方铺设了一层极轻的纯白外描边 `#FFFFFF`（线宽 `3.0px`，透明度 `35%`），旨在增强边界边缘的明晰度和立体感，避免了贴纸锯齿感，又不产生魔幻发光。
  - **行政区划边界**：采用浅灰绿色 `#C8CEC6`（极细 `0.8px`，不透明度 `50%`），保持极其微弱的视觉呈现，凸显低调稳重。
  - **管网线路层级**：主线路（1150kV/500kV/油气干线）统一使用低饱和深棕铜色 `#8A623B`；普通二级线路采用 `#A78257` 并降调至 `75%` 透明度，绝不遮挡和压过国家轮廓本身；外部规划联接则继续使用 `#9A9A9A` 灰色虚线。
  - **首府政务标识**：城市节点图标统一使用深蓝灰 `#264653`（首都 Astana 增附 `#C5A059` 细沙金外圈），标注字体则特调为深灰蓝 `#2F4858`，清晰易读且防止过黑突兀。
